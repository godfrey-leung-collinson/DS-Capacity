...
def fetch_region_definition(data_dir: Path, code_dir: Path) -> Path:
    """
    Extracting country and region definitions (including subregions)
    from Snowflake and exporting the results to a local directory
    ...
    try:
        ...
        with open(code_dir / "config/get_region_defn.yaml", "r") as f:
            config = yaml.safe_load(f)

        path_config = config["paths"]
        sql_template = directory / path_config["sql_template"]["get_region_defn"]

        logger.info(
            "Connecting to Snowflake and executing the queries for getting the country region definitions ..."
        )
        ...
        print(region_df.nunique())

        input("Check the extracted country and region definition data above. Press Enter to continue ...")

        logger.info("Exporting extracted region definitions ...")

        today = dt.date.today()

        filename = f"{filename_base}_{today}.csv"
        region_defn_filepath = str(output_parent_dir / f"{filename}")

        # export the results to local
        region_df.to_csv(
            region_defn_filepath,
            index=False,
        )
        ...
        logger.info("Finished extracting country region definitions from Snowflake.")
        logger.info(f"Time used = {end_time - start_time} seconds.")

        return region_defn_filepath
...
if __name__ == "__main__":
    ...
    region_defn_filepath = fetch_region_definition(directory / "data", directory / "analysis")
