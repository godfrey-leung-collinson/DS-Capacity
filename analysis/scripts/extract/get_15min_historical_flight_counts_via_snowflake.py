...
def update_sql_for_extracting_15min_historical_flight(
    ...
    Returns
    -------
        15-min interval historical flight counts (non-null/zero only) per airport
...
def fetch_15min_historical_flights(
    ...
    try:
        ...
        logger.info(
            "Connecting to Snowflake and executing the queries for counting 15min departure flights per airport and terminal ..."
        )
        ...
        for _ in range(days_to_run):
            ...
            final_sql_query = update_sql_for_extracting_15min_historical_flight(
                str(sql_template), start_date, temp_end_date, service_type
            )
        ...
        print(full_historical_flight_df.info())
        print(full_historical_flight_df.nunique())
        ...
        logger.info(
            "Finished extracting 15-min interval non-zero/null historical departure flight counts per airport and terminal."
        )
